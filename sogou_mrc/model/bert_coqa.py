...
class CharacterBert(BaseModel):
    ...
    def body(self, j, loss, token_index, token, y_pred):
        y_true = tf.where(tf.math.equal(tf.gather(token,tf.gather(token_index,j)),tf.gather(token, token_index)), tf.ones_like(token_index), tf.zeros_like(token_index))
        sim = keras.layers.Dot(axes=1, normalize=True)([tf.tile(tf.reshape(tf.gather_nd(y_pred,tf.reshape(tf.gather(token_index,j), [1,1])),[1,768]), tf.shape(token_index)),tf.gather_nd(y_pred, tf.reshape(token_index, [-1,1]))])
        return [tf.add(j,1), tf.add(loss,tf.keras.losses.BinaryCrossentropy(from_logits=False)(y_true, sim)), token_index, token, y_pred]
    
    def cond(self, j, loss, token_index, token, y_pred):  
        return tf.less(j,tf.size(token_index))

    def cl(self, y_pred, token):
        token_index = tf.where(tf.greater(token, tf.constant(0, dtype = tf.float32)))
        print(token_index)
        j = tf.constant(0, tf.int32)
        loss_cl = tf.constant(0, tf.float32)
        
        _, result, _, _, _ = tf.while_loop(self.cond, self.body, [j,loss_cl, token_index, token, y_pred])
        return result

    def body_sim(self, j, sim, token_index, token, y_pred):
        sim_new = keras.layers.Dot(axes=1, normalize=True)([tf.tile(tf.reshape(tf.gather_nd(y_pred,tf.reshape(tf.gather(token_index,j), [1,1])),[1,768]), tf.shape(token_index)),tf.gather_nd(y_pred, tf.reshape(token_index, [-1,1]))])
        return [tf.add(j,1), tf.concat([sim, sim_new], axis=0), token_index, token, y_pred]
    
    def cond_sim(self, j, sim, token_index, token, y_pred):  
        return tf.less(j,tf.size(token_index))

    def cl_sim(self, y_pred, token):
        token_index = tf.where(tf.equal(token, tf.constant(0, dtype = tf.float32)))
        print(token_index)
        j = tf.constant(0, tf.int32)
        sim = tf.zeros([1,1])
        _, result, _, _, _ = tf.while_loop(self.cond_sim, self.body_sim, [j,sim, token_index, token, y_pred], shape_invariants=[j.get_shape(), tf.TensorShape([None, 1]), token_index.get_shape(), token.get_shape(), y_pred.get_shape()])
        return result

    def _build_graph(self):
        self.training = tf.placeholder(tf.bool, shape=())
        #self.input_sim_matrix = tf.placeholder(shape=[None], dtype=tf.float32)
        self.token_isperson = tf.placeholder(shape=[None], dtype=tf.int32)
        ...
        dense = self.multi_linear_layer(final_hidden_matrix, self.n_layers, 768, 768,
                                                     activation='relu')

        self.loss = self.cl(dense, tf.cast(self.token_isperson, tf.float32))
        #self.sim_matrix = self.cl_sim(dense, tf.cast(self.token_isperson, tf.float32))
        #self.sim_matrix = tf.reshape(self.input_sim_matrix, tf.shape(self.input_sim_matrix))
        #self.loss_sim_matrix = tf.sqrt(tf.reduce_sum(tf.square(tf.subtract(self.input_sim_matrix, self.sim_matrix))))
        #self.final_loss = tf.add(self.loss, tf.multiply(self.loss_sim_matrix,tf.constant(self.coeff, dtype = tf.float32))
        self.input_placeholder_dict = OrderedDict({
            "input_ids": self.input_ids,
       #     "input_sim_matrix": self.input_sim_matrix,
            "token_isperson": self.token_isperson,
    ...
    def compile(self, learning_rate, num_train_steps, num_warmup_steps, use_tpu=False):
        #print('sncqowindoqiwndowqindoiwqdnoqwidnowqindowqidnwoqidn')
        #print(learning_rate)
    ...
    def train_and_evaluate_character(self, ratio, train_generator, eval_generator, evaluator, epochs=1, eposides=1,
        ...
        Trainer._train_and_evaluate_character(self, ratio, train_generator, eval_generator, evaluator, epochs=epochs,
                                    eposides=eposides,
